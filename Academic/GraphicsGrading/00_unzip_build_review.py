...
import numpy as np
### Local ###
from utils import ( read_config_into_env, env_get, out_line, levenshtein_search_dist, concat_structs,
                    tokenize, filter_float_tokens, vec_unit, angle_between, disp_text_header, TColor, )
from cHelper import ( dig_for_Makefile, source_and_header_full_paths, find_README, find_executable, 
                      make_clean, scrape_source, modify_makefile_to_disable_PIE, 
                      make_in_dir_from_rule_with_output, count_SLOC_comments, )
...
def file_EXT_full_paths( drctry, exts : str | list[str] ):
    """ Get paths to all files that match the list of `exts` """
    rtnPaths = deque()
    extMatch = exts[:] if isinstance( exts, list ) else [f"{exts}",]
...
def get_img_dims( imgPath : str ):
    """ Get the image dimensions """
    with Image.open( imgPath ) as im:
        return im.width, im.height


def p_image_square( imgPath : str ):
    """ Return True if the the image dimensions are identical """
    w, h = get_img_dims( imgPath )
    return (w == h)


def p_int_pow_2( n : int ):
    """ Return True if `n` is an even power of 2 """
    # Author: john-hen, https://stackoverflow.com/a/57027610
    return (n != 0) and (n & (n-1) == 0)


def p_img_dims_pow_2( imgPath : str ):
    """ Return True if the the image dimensions are an even power of 2 """
    w, h = get_img_dims( imgPath )
    ...
    def run_student_report( self, studentStr : str, studentDir : str, carryover : dict = None ):
        ...
        with open( reportPath, 'w' ) as f:
            ...
            if runStudent:
                ...
                print( f"Switching from {lastdir} --to-> {runDir}" )
                os.chdir( runDir ) # Change the current working directory
                os.system( f"./{execName}" )
                print( f"Switching from {runDir} --to-> {lastdir}" )
                os.chdir( lastdir ) # Restore the current working directory
            else:
                # 2025-10-17: Allow the program to access the local dir?
                lastdir = os.getcwd() # Cache the current working directory
                print( f"Switching from {lastdir} --to-> {hwDir}" )
                os.chdir( hwDir ) # Change the current working directory
                os.system( f"./{env_get('_GOOD_NAME')}" )
                print( f"Switching from {hwDir} --to-> {lastdir}" )
                os.chdir( lastdir ) # Restore the current working directory
            if not runStudent:
